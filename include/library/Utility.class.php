...
class Utility 
{
    ...
    static public function HumanTime($time=null, $forceDate=false)
    {
        ...
        if ( $forceDate || $interval > 30*86400 ){
            return self::GetRussianDate($time, 1);
        }else if ( $interval > 86400 ){
            $number = intval($interval/86400);
            return "${number} ".self::GetNormalWord($number, "день", "дня", "дней")." назад";
        }else if ( $interval > 3600 ){ // > 1 hour
            $number = intval($interval/3600);
            return "${number} ".self::GetNormalWord($number, "час", "часа", "часов")." назад";
        }else if ( $interval >= 60 ){ // > 1 min
            $number = intval($interval/60);
            return "${number} ".self::GetNormalWord($number, "минуту", "минуты", "минут")." назад";
        }else if ( 5 >= $interval){// < 5 second
            return "только что";
        }else{ // < 1 min
            return "${interval} ".self::GetNormalWord($interval, "секунду", "секунды", "секунд")." назад";
        }
    }
    ...
    static public function GetRussianDate($date = null, $y = null) {
	if ($date){
	...
	return abs(intval($date[0]))." ".$aMonth[$date[1] - 1].$y;
    }

    /*
    Функция вызывается с 4-мя параметрами:
    1) Число к которому надо проставить нужно окончание
    2) Вариант для единицы (ящик, сотка)
    3) Вариант для малого количества (ящика, сотки)
    4) Вариант для большого количества (ящиков, соток)
    Для работы функции не требуются никакие модули и PHP может быть скомпилирован с минимальном набором функций
    */

    /*static public function GetNormalWord($t,$a1=" день",$a2=" дня",$a3=" дней") {
    ...
    static public function GetNormalWord($REZahl=0,$REVar1=" день",$REVar2=" дня",$REVar3=" дней") {
