...
include_once(dirname(dirname(__FILE__)) . '/lib/iqsms_function.php');
		...
		if(isset($date) && isset($time) && isset($master_name) && isset($street)){
			if($date != '' && $time != '' && $master_name != '' && $street != ''){
				...
				if($date_f <= 82000 && $date_f >= 0){
					//Отправляем смс
					prepare_sms_master($order_id);
					prepare_sms_client($order_id, $phone, $client_fio, 3);
					if(isset($_POST['sms2']) && $_POST['sms2'] == 'on'){
						prepare_sms_client($order_id, $phone2, $client_fio2, 4);
					}
					if(isset($_POST['sms3']) && $_POST['sms3'] == 'on'){
						prepare_sms_client($order_id, $phone3, $client_fio3, 5);
					}
		...
		if($master_name_two != ''){
			...
			if(isset($date) && isset($time) && isset($master_name_two) && isset($street)){
				if($date != '' && $time != '' && $master_name_two != '' && $street != ''){
					...
					$date_f = date_diff_f($time_now, $date);
					if($date_f <= 82000 && $date_f >= 0){
						prepare_sms_client($order_id_two, $phone, $client_fio, 3);
						//Отправляем смс
						prepare_sms_master($order_id_two);
						if(isset($_POST['sms2']) && $_POST['sms2'] == 'on'){
							prepare_sms_client($order_id_two, $phone2, $client_fio2, 4);
						}
						if(isset($_POST['sms3']) && $_POST['sms3'] == 'on'){
							prepare_sms_client($order_id_two, $phone3, $client_fio3, 5);
						}
					}					
				}
			}
